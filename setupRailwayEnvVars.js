/**
 * Setup Essential Railway Environment Variables
 * 
 * This script generates the essential environment variables needed for Railway deployment.
 * It outputs commands that can be run to set the variables using the Railway CLI.
 */

const crypto = require('crypto');

// Generate secure random values
const generateSecureToken = (length = 32) => {
  return crypto.randomBytes(length).toString('hex');
};

// Essential environment variables
const essentialVars = [
  {
    name: 'JWT_SECRET',
    value: '*ogMU&%cx!u8iUw^KgGDA8neFF@oGMA9&9U^Cr8Jzb2o6dMqYc#4iSbz2X%x$Xc4',
    description: 'Secret key for JWT token generation'
  },
  {
    name: 'JWT_EXPIRES_IN',
    value: '7d',
    description: 'JWT token expiration time'
  },
  {
    name: 'JWT_COOKIE_EXPIRES_IN',
    value: '90',
    description: 'JWT cookie expiration time in days'
  },
  {
    name: 'NODE_ENV',
    value: 'production',
    description: 'Environment mode'
  },
  {
    name: 'CORS_ORIGIN',
    value: 'https://blathers.app',
    description: 'Allowed CORS origins'
  },
  {
    name: 'GUEST_LEADERBOARD_TOKEN',
    value: 'a7b9c2d5e8f3g6h1j4k7m2n5p8r3t6v9',
    description: 'Token for guest leaderboard access'
  },
  {
    name: 'GUEST_IMAGE_TOKEN',
    value: 'a7b9c2d5e8f3g6h1j4k7m2n5p8r3t6v9',
    description: 'Token for guest image access'
  },
  {
    name: 'SESSION_SECRET',
    value: 'acnh_quiz_session_production_secret_2024_blathers',
    description: 'Secret for session management'
  },
  {
    name: 'NOOKIPEDIA_API_KEY',
    value: '6f2b7c8f-9a1d-4e3c-b0f2-e5d4c3b2a1f0',
    description: 'Nookipedia API key'
  }
];

// Print instructions
console.log('='.repeat(80));
console.log('RAILWAY ENVIRONMENT VARIABLES SETUP');
console.log('='.repeat(80));
console.log('\nTo set up environment variables in Railway:');
console.log('\nOption 1: Using Railway CLI');
console.log('Run the following commands:');
console.log('\n');

// Generate CLI commands
essentialVars.forEach(envVar => {
  console.log(`railway variables set ${envVar.name}="${envVar.value}"`);
});

console.log('\n');
console.log('Option 2: Using Railway Dashboard');
console.log('1. Go to the Railway dashboard: https://railway.app/dashboard');
console.log('2. Select your project "blathers.app"');
console.log('3. Click on your service (Capstone-Project)');
console.log('4. Go to the "Variables" tab');
console.log('5. Add the following environment variables:');
console.log('\n');

// Print environment variables table
console.log('| Variable Name | Value | Description |');
console.log('|---------------|-------|-------------|');

essentialVars.forEach(envVar => {
  const value = envVar.name.includes('SECRET') || envVar.name.includes('TOKEN') || envVar.name.includes('KEY') 
    ? '********' // Mask sensitive values
    : envVar.value;
  
  console.log(`| ${envVar.name.padEnd(20)} | ${value.padEnd(10)} | ${envVar.description} |`);
});

console.log('\n');
console.log('Note: The MONGODB_URI variable should be automatically set by Railway when you created the MongoDB database.');
console.log('\n');
console.log('After setting these variables, redeploy your application:');
console.log('railway up');
console.log('\n');
console.log('='.repeat(80));
